...
import pendulum
from monpetitsalon.utils import replace_if_error, get_item
...
class BaseItemParser:
    ...
    def get_item(self, soup, item_key):
        item = get_item(soup, self.data_paths.get(item_key),
    ...
    def parse_raw_item(self, soup):
        parsed_raw_item = {item_key: self.get_item(
            soup, item_key) for item_key in self.data_paths.keys()}
    ...
    def parse_modification_date(self, soup):
        modification_dates = [
            pendulum.from_format(
                e['title'].replace('1er', '1'), 'D MMMM YYYY HH:mm:ss', locale='fr')
